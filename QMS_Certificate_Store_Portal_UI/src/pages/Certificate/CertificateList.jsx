...
const ViewModal = ({ cert, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
            <div className="bg-card border border-border rounded-2xl w-full max-w-xl shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-300">
                <div className="flex items-center justify-between px-6 py-4 border-b border-border">
                    <div className="flex items-center gap-2">
                        <Award size={18} className="text-[#D4A95A]" />
                        <span className="font-black uppercase tracking-widest text-sm text-foreground">Certificate Details</span>
                    </div>
                ...
                <div className="p-6 space-y-4">
                    <div>
                        <p className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground font-black mb-1">Certificate</p>
                        <p className="text-xl font-black text-foreground">{cert.certificateName}</p>
                        <p className="text-[11px] text-muted-foreground uppercase tracking-widest mt-0.5">{cert.certificateNumber}</p>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        {[
                            ...
                            <div key={label} className="bg-muted/20 rounded-xl px-4 py-3 border border-border/50">
                    ...
                    {cert.notes && (
                        <div className="bg-muted/20 rounded-xl px-4 py-3 border border-border/50">
                            <p className="text-[9px] font-black uppercase tracking-[0.25em] text-muted-foreground mb-1">Notes</p>
                            <p className="text-sm text-foreground/80">{cert.notes}</p>
                        </div>
                    )}
                    {cert.filePath && (
        ...
        <div className="fixed inset-0 z-[998] flex justify-end" onClick={onClose}>
            <div className="h-full w-80 bg-card border-l border-border shadow-2xl animate-in slide-in-from-right duration-300 overflow-y-auto"
                ...
                <div className="p-5 space-y-4">
                    <p className="font-black text-foreground text-base">{cert.certificateName}</p>
                    <p className="text-[10px] uppercase tracking-widest text-muted-foreground">{cert.certificateNumber}</p>
                    {[
                        ['Type', cert.certificateTypeName],
                        ['Owner', cert.ownerName],
                        ['Department', cert.departmentName],
                    ...
                    {cert.notes && (
                        <div>
                            ...
                            <p className="text-sm text-foreground/80 mt-1">{cert.notes}</p>
                        </div>
                    )}
                </div>
            </div>
...
const CertificateList = () => {
    ...
    const TH = ({ ch, cls = '' }) => (
        <th className={`px-3 py-3.5 text-[9px] font-black uppercase tracking-[0.22em] text-muted-foreground whitespace-nowrap ${cls}`}>{ch}</th>
    );
        ...
        <div className="p-6 space-y-5 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <h1 className="text-3xl font-black text-foreground flex items-center gap-3">
                    <Award className="text-gold" size={32} /> Certificates Registry
                </h1>
                            ...
                            {loading ? (
                                ...
                                currentItems.map((item) => (
                                    <tr
                                        ...
                                        className="group hover:bg-gold/[0.03] transition-colors duration-100"
                                    >
                                            ...
                                            <div className="flex items-center justify-center gap-1">
                                                ...
                                                {item.filePath ? (
                                                    <button
                                                        ...
                                                        title="Download"
                                                        className="w-7 h-7 rounded-lg bg-muted/50 hover:bg-emerald-500/20 hover:text-emerald-500 text-muted-foreground flex items-center justify-center transition-all"
                                                    >
                                                    ...
                                                    <button
                                                        disabled
                                                        title="No file"
                                                        className="w-7 h-7 rounded-lg bg-muted/20 text-muted-foreground/30 flex items-center justify-center cursor-not-allowed"
                                                    >
                                                        <FileText size={13} />
                                                    </button>
                                                )}
                                                ...
                                                <button
                                                    ...
                                                    title="History"
                                                    className="w-7 h-7 rounded-lg bg-muted/50 hover:bg-blue-500/20 hover:text-blue-500 text-muted-foreground flex items-center justify-center transition-all"
                                                >
                                                ...
                                                {canEdit && (
                                                    <button
                                                        ...
                                                        title="Edit"
                                                        className="w-7 h-7 rounded-lg bg-gold/10 hover:bg-gold/30 text-gold flex items-center justify-center transition-all"
                                                    >
